...
2. **If no rule set exists, explore the whole repo and document current practice as its own artifact — don't skip straight to comparison.** Walk the actual tree — every top-level module/package/layer, not just the files that happen to be open or recently changed. For each layer, read config files (linter/formatter configs, `package.json`/`pyproject.toml`/etc.), a representative file or two, and tests if present. Produce a **"Current codebase conventions" table** — one row per category from `references/language-specific-standards.md` (naming, formatting, type system usage, imports, idioms, error handling, comments, dependency management) plus file/folder structure and testing patterns — describing what this repo actually does for each, with example file references. This table is the deliverable of this step; don't proceed to step 3 with only an informal mental impression.

   **Flag internal inconsistencies as their own line item**, separate from any external comparison: where a convention is genuinely mixed across the repo (e.g. two error-handling styles coexist, some modules use one naming case and others another), record which files/areas use which variant, and note if one is clearly newer (check git history/recency) and thus the likely intended direction. Internal inconsistency is worth flagging on its own merits — a codebase that doesn't even follow itself is a problem regardless of what any external standard says.

3. **Identify the stack and look up the relevant big-company standard.** Detect language(s)/framework(s) in use, then web-search the standard style guide(s) that apply (e.g. Google's JS/TS or Python style guide, Airbnb JS, PEP 8, Go's Effective Go, etc. — whichever fits the detected stack). Work through the same `references/language-specific-standards.md` categories used to build the current-practice table in step 2, so the two line up category-for-category for the comparison in step 4.

4. **Compare the step-2 current-practice table against the step-3 big-company standard, category by category, and flag two distinct things:**
   - **Alignment/divergence vs. the external standard** — for each category, does the codebase already follow the big-company recommendation, or diverge from it? Classify every divergence by effort AND impact:
   ...
   - **Internal inconsistencies carried over from step 2** — these need a decision too (which variant becomes the standard going forward), independent of whether either variant matches the external big-company guide.

   Never decide either bucket silently, but weight the discussion toward "Consider later" and inconsistencies: present a table — **Rule | Current practice | Big-company recommendation | Effort | Impact | Suggested verdict** — and for every "Consider later" row or flagged inconsistency, wait for an explicit per-row decision (adopt / defer / reject / pick-one-variant) rather than accepting a blanket "looks good" as covering it. Blanket agreement is fine for the "Adopt now" rows only.
